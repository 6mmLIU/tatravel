...
import cn.sixmm.sixsixsix.business.api.domain.*;
import cn.sixmm.sixsixsix.business.mapper.StrategyCatalogMapper;
import cn.sixmm.sixsixsix.business.mapper.StrategyContentMapper;
import cn.sixmm.sixsixsix.business.mapper.StrategyMapper;
import cn.sixmm.sixsixsix.business.mapper.StrategyThemeMapper;
import cn.sixmm.sixsixsix.business.query.StrategyQuery;
...
import cn.sixmm.sixsixsix.business.util.DateUtil;
import cn.sixmm.sixsixsix.business.vo.CatalogVO;
...
@Service
@Transactional
public class StrategyServiceImpl extends ServiceImpl<StrategyMapper, cn.sixmm.sixsixsix.business.api.domain.Strategy> implements IStrategyService {
    ...
    @Override
    public IPage<cn.sixmm.sixsixsix.business.api.domain.Strategy> queryPage(StrategyQuery qo) {
        IPage<cn.sixmm.sixsixsix.business.api.domain.Strategy> page = new Page<>(qo.getCurrentPage(), qo.getPageSize());
        LambdaQueryWrapper<cn.sixmm.sixsixsix.business.api.domain.Strategy> qw = new LambdaQueryWrapper<>();
        if (qo.getType() != null) {
            if (qo.getType() == 1 || qo.getType() == 2) {
                qw.eq(cn.sixmm.sixsixsix.business.api.domain.Strategy::getDestId, qo.getRefid());
            } else if (qo.getType() == 3) {
                qw.eq(cn.sixmm.sixsixsix.business.api.domain.Strategy::getThemeId, qo.getRefid());
            }
        }
    ...
    public List<CatalogVO> queryCatalogVO(Long destId) {
        LambdaQueryWrapper<cn.sixmm.sixsixsix.business.api.domain.StrategyCatalog> qw = new LambdaQueryWrapper<>();
        qw.eq(cn.sixmm.sixsixsix.business.api.domain.StrategyCatalog::getDestId, destId);
        List<cn.sixmm.sixsixsix.business.api.domain.StrategyCatalog> list = strategyCatalogMapper.selectList(qw);
        List<CatalogVO> vos = new ArrayList<>();
        for (cn.sixmm.sixsixsix.business.api.domain.StrategyCatalog strategyCatalog : list) {
            CatalogVO vo = new CatalogVO();
            vo.setId(strategyCatalog.getId());
            vo.setName(strategyCatalog.getName());
            List<cn.sixmm.sixsixsix.business.api.domain.Strategy> strategies = lambdaQuery()
                    .eq(cn.sixmm.sixsixsix.business.api.domain.Strategy::getCatalogId, strategyCatalog.getId())
                    .list();
    ...
    @Override
    public List<cn.sixmm.sixsixsix.business.api.domain.Strategy> queryViewnumTop3(Long destId) {
        return lambdaQuery()
                .eq(cn.sixmm.sixsixsix.business.api.domain.Strategy::getDestId, destId)
                .orderByDesc(cn.sixmm.sixsixsix.business.api.domain.Strategy::getViewnum)
                .last("limit 3")
                .list();
    }

    @Override
    public int saveStrategy(cn.sixmm.sixsixsix.business.api.domain.Strategy strategy) {
        cn.sixmm.sixsixsix.business.api.domain.StrategyTheme theme = themeMapper.selectById(strategy.getThemeId());
        strategy.setThemeName(theme.getName());

        cn.sixmm.sixsixsix.business.api.domain.StrategyCatalog catalog = catalogMapper.selectById(strategy.getCatalogId());
...
//
    ...
    public List<ThemeVO> queryTheme() {
        QueryWrapper<cn.sixmm.sixsixsix.business.api.domain.Strategy> qw = new QueryWrapper<>();
    ...
    private void saveRank(Date now, long type) {
        List<cn.sixmm.sixsixsix.business.api.domain.Strategy> list;
        if (type == 1) {
            list = lambdaQuery().eq(cn.sixmm.sixsixsix.business.api.domain.Strategy::getIsabroad, 1)
                    .orderByDesc(cn.sixmm.sixsixsix.business.api.domain.Strategy::getViewnum)
                    .last("limit 10").list();
        } else if (type == 2) {
            list = lambdaQuery().eq(cn.sixmm.sixsixsix.business.api.domain.Strategy::getIsabroad, 0)
                    .orderByDesc(cn.sixmm.sixsixsix.business.api.domain.Strategy::getViewnum)
                    .last("limit 10").list();
        } else {
            list = lambdaQuery().orderByDesc(cn.sixmm.sixsixsix.business.api.domain.Strategy::getViewnum)
                    .last("limit 10").list();
        }
